...
There are 3,929 bookings of resort hotels with 1+ children or babies.
...
There are 5,403 bookings of city hotels with 1+ children or babies (more than resort hotel bookings).
...
The most common number of adults in bookings is 2. It is surprising that there are 403 bookings for 0 adults.
...
```{r freq-canceledbookings}
hotels %>%
  filter(is_canceled == 1) %>%
  count(adults)
```

```{r freq-notcanceledbookings}
hotels %>%
  filter(is_canceled == 0) %>%
  count(adults)
```

More not cancelled bookings (vs. cancelled) are booked for 0 adults.
